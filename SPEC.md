...
| V53 | desktop `.decks-list` is CSS grid `repeat(auto-fill, minmax(180px, 1fr))` with `gap: 8px` — auto wraps to multi-column whenever decks panel is wide enough; mobile (<800px) stays single-column flex (one tile per row, full-width); container query on `.decks-list` scales deck-stack per V81 — supersedes prior 280px minmax |
...
| V81 | deck-tile `.deck-stack` size scales by `.decks-list` container width (CSS container query): default `60×90` outer / `50×78` inner layer; `@container (min-width: 360px)` → `78×114` outer / `66×102` inner (matches V78 desktop sizing); mobile (<800px) keeps `60×90` baseline. Supersedes the desktop scaling clause of V78 — V78 numerics retained but now gated by container width, not viewport `@media` |
| V82 | desktop (≥800px) `.inventory-panel` distributes its three `.right-subpanel` children equally over full vertical height: each child `flex: 1 1 0; min-height: 0`; sub-panel `border-bottom`/`padding-bottom` separators stripped on desktop only (mobile retains V68 separators); sub-panel content stays top-aligned within its share (no internal centering); each sub-panel scrolls independently when content exceeds its share |
...
| T80 | . | container query on `.decks-list`: `container-type: inline-size; container-name: decks-list`; default `.deck-stack` `60×90` outer / `50×78` inner; `@container decks-list (min-width: 360px)` → `78×114` outer / `66×102` inner (scale offset transforms accordingly); remove existing `@media (min-width: 800px) .deck-stack`/`.deck-stack-layer` size rules at line ~257 in `style.css` | V81,I.ui |
| T81 | . | desktop (`@media (min-width: 800px)`) `.inventory-panel`: `gap: var(--gap)`; `.right-subpanel { flex: 1 1 0; min-height: 0; border-bottom: none; padding-bottom: 0 }`; verify even vertical distribution at 720p / 1080p / 1440p tall viewports; verify per-subpanel scroll when summary list / discard list overflows | V82,I.ui |
...
| B12 | 2026-05-10 | `.decks-list` `minmax(280px,1fr)` keeps decks single-column up to ~1280px viewport — wastes horizontal space at large monitors; right-rail subpanels (`Inventory`/`Discard History`/`Quick Help`) stack tight at top of `.inventory-panel` with `gap: 10px`, leaving large empty area below at tall viewports | V53 amended (180px); V81 container-query deck-stack scaling; V82 right-rail equal vertical distribution; T79–T82 |
